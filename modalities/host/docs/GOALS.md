# host GOALS

- [ ] Keep a live, attention-packed process that ranks modalities by parent-goal effectiveness under bytes
- [ ] **Author and refresh scoring models** (modality effectiveness · jmethods · [[ceo_score_models]]) so Best is not an echo
- [ ] Maintain hop0 with loop phase always present
- [ ] Surface externals for outer author to probe new modalities
- [ ] Advance densest roadmap without note farm
- [ ] Prefer densest parent-goal help over minimum loop latency (thorough when needed)
- [ ] When human parks REVIEW, keep exploring under `review_park_explore` without inventing joy values
